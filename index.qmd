...
p_memorability <- targets::tar_read(memorability) |>
  ...
  geom_linerange(color = "grey", linewidth = 0.2) +
...
med_diagram <- function(data, height = .75, width = 2, graph_label = NA,
                        ...
                        ranksep = .2, minlen = 3, output_path = NULL) {
  ...
  grViz_diagram <- DiagrammeR::grViz(diagram_out)

  # Save as PNG if output_path is specified
  if (!is.null(output_path)) {
    svg <- DiagrammeRsvg::export_svg(grViz_diagram) # Convert to SVG
    rsvg::rsvg_png(charToRaw(svg), output_path) # Convert SVG to PNG and save
  }

  return(grViz_diagram)
}
  ...
  mutate(
    plot = walk2(
      ...
      \(cca_id, med_data) {
        med_diagram(
          med_data,
          output_path = glue::glue("figures/mediation_diagram_{cca_id}.png")
        )
      }
    )
  )

...
arrow::read_parquet(targets::tar_read(file_rs_group_whole)) |>
  ...
  filter(index_name %in% names(labels_index_name)) |>
  ggplot(aes(fisher_z, score, color = index_name)) +
  geom_point() +
...
arrow::read_parquet(targets::tar_read(file_rs_group_trial)) |>
  ...
  inner_join(mem_perf, by = "subj_id", relationship = "many-to-many") |>
  filter(index_name %in% names(labels_index_name)) |>
  ggplot(aes(mean_fisher_z, score, color = index_name)) +
  geom_point() +
  stat_smooth(method = "lm", formula = y ~ x) +
  ggpmisc::stat_correlation(
    ggpmisc::use_label(c("r", "p.value")),
...
```{r}
#| label: fig-scatter-mem-perf
#| fig-cap: Visualize correlation between familiarity and recollection
...
mem_perf |>
  ...
  scale_x_continuous(name = labels_index_name["knowadj"]) +
  scale_y_continuous(name = labels_index_name["remember"]) +
  theme_bw()
```

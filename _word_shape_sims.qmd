...
file_words = os.path.join("data", "stimuli", "words_list.tsv")
file_sims = os.path.join("data", "stimuli", "word_shape_sims", "char_similar.tsv")
words = pd.read_csv(file_words, sep="\t")["word"][:150].to_numpy()
...
for row in tqdm(sim_scores.itertuples()):
...
sim_scores.to_csv(file_sims, sep="\t", index=False)
```
...
path_images = os.path.join("data", "stimuli", "images")
file_sims = os.path.join("data", "stimuli", "word_shape_sims", "alexnet.tsv")
...
# save to a tsv file
similarities.to_csv(file_sims, sep="\t", index=False)
```

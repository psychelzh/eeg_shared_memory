...
rsa_inter_by_trial <- arrow::read_parquet("data/task-rs_type-inter_acq-trial.parquet")
...
rsa_group_whole <- arrow::read_parquet("data/task-rs_type-group_acq-whole.parquet")
rsa_group_whole |> 
  mutate(z_fisher = atanh(r)) |> 
  inner_join(mem_ability, by = "subj_id", relationship = "many-to-many") |>
  filter(mem_type != "know") |> 
  ggplot(aes(z_fisher, dprime, color = mem_type)) +
  geom_point() +
...
rsa_group_by_trial <- arrow::read_parquet("data/task-rs_type-group_acq-trial.parquet")
rsa_group_by_trial |> 
  summarise(
    mean_z_fisher = mean(atanh(r), na.rm = TRUE),
    .by = c(region_id, subj_id)
  ) |> 
  inner_join(mem_ability, by = "subj_id", relationship = "many-to-many") |>
  filter(mem_type != "know") |> 
  ggplot(aes(mean_z_fisher, dprime, color = mem_type)) +
  geom_point() +
  stat_smooth(method = "lm", formula = y ~ x) +
  ggpmisc::stat_correlation(
    ggpmisc::use_label(c("r", "p.value")),
